...
namespace FrontendService.Test.Unit.Seeding
{
    ...
    public class EventReplayerTest
    {
        ...
        public void ReplayEvents_CallsReplayEventsAsyncOnAuditAgent(string topic, Type type)
        {
            ...
            IEventReplayer eventReplayer = new EventReplayer(serviceCollection, auditAgentMock.Object, new NullLoggerFactory());

            auditAgentMock.Setup(e =>
                e.ReplayEventsAsync(It.Is<ReplayEventsCommand>(c => c.EventType == type.Name && c.TopicFilter == topic)))
                .ReturnsAsync("0")
                .Verifiable();
